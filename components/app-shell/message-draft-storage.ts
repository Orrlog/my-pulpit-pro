...
const defaultSeeds: MovementSeed[] = [
  ...
  {
    title: "Let the Heart Be Exposed",
  ...
  {
    title: "Leave with a Clear Response",
...
export function buildIntroduction(input: {
  ...
  return {
    hook: `Open with a real moment connected to ${input.directionTitle}: the place where faith sounds simple until life puts pressure on it.`,
    pastoralTension: `${angle} Say it plainly, without scolding people for feeling the weight of it.`,
    passageConnection: `Bring the church to ${input.mainScripture} and let the passage set the path for the sermon.`,
    bigIdeaBridge: cleanSentence(input.bigIdea),
    firstMovementTransition: "Move from the pressure people recognize into the first truth the passage gives them.",
  };
}
...
export function buildClosing(input: {
  ...
  return {
    ...
    callToResponse: "Choose one honest next step that fits the Word you have heard today.",
